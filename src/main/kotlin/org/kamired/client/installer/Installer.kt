package org.kamired.client.installer

import org.kamiblue.client.util.WebUtils
import org.kamired.client.KamiMod
import org.kamired.client.util.filesystem.FolderUtils
...
object Installer : JPanel() {
    ...
    fun main(args: Array<String>) {
        ...
        if (downloadsApi.size < 19) {
            notify("Error while loading the KAMI Red Downloads API, couldn't connect to the URL or response is invalid. " +
        ...
        val logoFile = Installer.javaClass.getResource("/installer/kami.png")
            frame = JFrame("KAMI Red Installer")
        ...
        if (kamiJars.size > 0) {
            val action = confirm("Attention! It looks like you have installed KAMI Red before. Do you want to delete these older versions?")
    ...
    init {
        ...
        stableButton.toolTipText = "This version of KAMI Red is the latest major release"
        betaButton.toolTipText = "A beta version of KAMI Red, with frequent updates and fixes"
        ...
        betaButton.addActionListener {
            ...
            download(betaUrl)
            notify("KAMI Red $betaVersion has been installed", "Installed", JOptionPane.INFORMATION_MESSAGE)
            exitProcess(0)
        }
        ...
        stableButton.addActionListener {
            ...
            betaButton.isOpaque = false
            download(stableUrl)
            notify("KAMI Red $stableVersion has been installed", "Installed", JOptionPane.INFORMATION_MESSAGE)
            exitProcess(0)
        }
    }
    ...
    private fun notify(message: String, title: String = "KAMI Red Installer", type: Int = JOptionPane.WARNING_MESSAGE) {
    ...
    private fun download(url: String) {
        ...
        Thread {
            dialog[0] = JOptionPane("", JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION)
                .createDialog(null, "KAMI Red - Downloading")
